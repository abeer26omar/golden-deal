...
import { Address, Addresses, APIResponse } from '../user.model';
...
export class AddressesService {
  ...
  addNewAddress(title: string,address_type: string,city:string,street:string,building:string){
    return this.http.post(`${env.api_url}/address/store`,{
      title:title,
  ...
  updateAddress(address_id: number,title: string,address_type: string,city: string,street: string,building: string){
    return this.http.post(`${env.api_url}/address/update`,{
  ...
  deleteAdd(id:number){
    return this.http.get(`${env.api_url}/address/delete/${id}`,this.httpOptions).pipe(tap(()=>{
      this._refresh.next();
    }))
  }
  setPrimary(id: number){
    return this.http.get(`${env.api_url}/address/set-primary/${id}`,this.httpOptions).pipe(tap(()=>{
      this._refresh.next();
    }))
  }
}
